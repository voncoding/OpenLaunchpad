...
- 壁纸读取失败时显示原因和重试入口，避免无提示的纯黑背景。
- 照片壁纸配置被系统拦截时提供隐私设置入口，说明完整磁盘访问的范围及改用本地图片的替代方式。
- 普通图片壁纸跳过受保护的照片缓存；照片壁纸仅按当前选中的标识查找，区分权限不足与缓存缺失。
- 新增五项壁纸来源回归检查，交互回归共 56 项。
- 修复首次直接打开设置时壁纸状态一直停留在读取中的问题；图片解码失败也会显示错误。

macOS 27 上，部分“照片”来源壁纸的配置受系统保护。若设置显示权限不足，可前往“系统设置 → 隐私与安全性 → 完整磁盘访问”添加当前安装的启动台，再退出并重新打开。该权限范围大于壁纸目录；也可以在系统壁纸设置中改用本地图片文件。本次更新不绕过系统权限。

安装包包含 Apple Silicon 与 Intel 两种架构，使用临时签名，尚未经过 Apple 公证。替换旧版后，系统可能要求重新授权；请以更新后的应用为准。
